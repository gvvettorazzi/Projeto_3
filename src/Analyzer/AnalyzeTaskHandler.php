...
{
try {
$sourceFile = $this->validateSourceFile($task->sourceFile);
$content = $this->readSourceFileSecurely($sourceFile);
if (!mb_check_encoding($content, 'UTF-8')) {
return [
new ErrorInfo(
ErrorKind::InvalidEncoding,
'Source file has an unsupported character encoding.',
),
];
}
...
$this->processNodes($task, $ast),
preserve_keys: false,
);
} catch (\PhpParser\Error) {
/*
* CONFIDENTIALITY:
* Never expose parser messages because they may contain
* source-code fragments, file paths or implementation details.
*/
return [
new ErrorInfo(
ErrorKind::SyntaxError,
...
'Source file could not be analyzed.',
),
];
}
}
/**
* Validates an analyzer input path before accessing the filesystem.
*
* Confidentiality and defensive-input controls:
* - rejects empty paths;
* - rejects null bytes;
* - bounds path length;
* - resolves the canonical path;
* - only accepts regular readable files.
*/
private function validateSourceFile(string $sourceFile): string
{
if (
$sourceFile === ''
|| strlen($sourceFile) > 4096
|| str_contains($sourceFile, "\0")
) {
...
/**
* Reads source code using a bounded input size.
*
* Prevents malformed or excessively large inputs from propagating
* unchecked through the analyzer.
*/
private function readSourceFileSecurely(string $sourceFile): string
{
...
$task->primary =
$task->primary
&& $this->filter->filterClassLikeNode($node);
$classLike = $this->processClassLike(
$task,
$node,
);
yield $classLike;
yield from $this->extractDependencies(
$classLike,
);
} catch (\Throwable $e) {
/*
* Do not include namespacedName in the message.
* The original exception remains chained internally,
* but it is never exposed by handle().
*/
throw new \LogicException(
'Class-like analysis failed.',
0,
$e,
);
}
...
try {
$functionInfo = $this->processFunction(
$task,
$node,
);
...
// TODO: constants, class aliases
foreach ($node->getSubNodeNames() as $name) {
$subNode = $node->$name;
if (is_array($subNode)) {
yield from $this->processNodes(
$task,
$subNode,
);
...
if (
$task->primary
&& !$this->filter->filterClassLikeTags($tags)
) {
$task->primary = false;
}
...
: null;
$info->implements = $this->processNameList(
$node->implements,
$tags,
$implementsTagNames,
);
$info->aliases = $this->extractAliases(
$classDoc,
$node->getDocComment()?->getStartLine(),
$node->getDocComment()?->getEndLine(),
);
unset(
$tags['phpstan-type'],
$tags['phpstan-import-type'],
);
foreach ($node->getTraitUses() as $traitUse) {
$info->uses += $this->processNameList(
$traitUse->traits,
$tags,
$useTagNames,
);
...
$info->extends = $this->processNameList(
$node->extends,
$tags,
$extendsTagNames,
);
...
$autoImplement = new ClassLikeReferenceInfo(
$node->scalarType
? BackedEnum::class
: UnitEnum::class,
);
...
throw new \LogicException(
sprintf(
'Unsupported ClassLike node %s',
get_debug_type($node),
),
);
}
$info->genericParameters =
$this->extractGenericParameters($classDoc);
$info->description =
$this->extractMultiLineDescription($classDoc);
$info->tags = $tags;
/*
* Keep the original sourceFile here because ApiGen relies on it
* internally for source references. The value is no longer exposed
* through public error messages.
*/
...
unset(
$info->tags['property'],
$info->tags['property-read'],
$info->tags['property-write'],
);
unset($info->tags['method']);
if (
$info->primary
&& !$this->filter->filterClassLikeInfo($info)
) {
$info->primary = false;
}
return $info;
...
* @return iterable<MemberInfo>
*/
protected function extractMembersFromBody(
Node\Stmt\ClassLike $node,
): iterable
{
foreach ($node->stmts as $member) {
$memberDoc =
$this->extractPhpDoc($member);
$description =
$this->extractMultiLineDescription(
$memberDoc,
);
$tags =
$this->extractTags($memberDoc);
/*
* CONFIDENTIALITY BOUNDARY:
* Members excluded by tags are discarded before their
* contents are transformed into public documentation.
*/
...
if (
!$this->filter->filterConstantNode(
$member,
)
) {
continue;
...
$memberInfo->tags =
$tags;
$memberInfo->startLine =
$this->getMemberStartLine(
$member,
);
$memberInfo->endLine =
$member->getEndLine();
...
if (
!$this->filter->filterPropertyNode(
$member,
)
) {
continue;
...
$varTag =
isset($tags['var'][0])
&& $tags['var'][0] instanceof VarTagValueNode
? $tags['var'][0]
: null;
unset($tags['var']);
foreach ($member->props as $property) {
$memberInfo = new PropertyInfo(
$property->name->name,
);
$memberInfo->description =
$this->extractSingleLineDescription(
$varTag,
);
...
$returnTag =
isset($tags['return'][0])
&& $tags['return'][0] instanceof ReturnTagValueNode
? $tags['return'][0]
: null;
unset(
$tags['param'],
...
/*
* Defensive validation instead of relying only
* on assertions, which may be disabled.
*/
if (
!$param->var
instanceof Node\Expr\Variable
...
$propertyInfo->startLine =
$param->getStartLine();
$propertyInfo->endLine =
$param->getEndLine();
$propertyInfo->public =
(bool) (
$param->flags
...
if (
!$this->filter->filterEnumCaseNode(
$member,
)
) {
continue;
}
$memberInfo = new EnumCaseInfo(
$member->name->name,
$this->processExprOrNull(
$member->expr,
),
...
/**
* Returns the first relevant source line without assuming
* that a comment is present.
*/
private function getMemberStartLine(Node $node): int
{
...
* @return iterable<MemberInfo>
*/
protected function extractMembersFromTags(
array $tags,
): iterable
{
$propertyTags = [
...
{
if (
!$this->filter->filterFunctionNode($node)
) {
return null;
}
$phpDoc =
$this->extractPhpDoc($node);
$tags =
$this->extractTags($phpDoc);
if (
!$this->filter->filterFunctionTags($tags)
) {
return null;
}
assert($node->namespacedName !== null);
...
$node->byRef;
if (
!$this->filter->filterFunctionInfo($info)
) {
return null;
}
return $info;
...
/*
* Do not depend exclusively on assert().
* Assertions may be disabled in production.
*/
if (
!$parameter->var
instanceof Node\Expr\Variable
|| !is_scalar($parameter->var->name)
) {
continue;
}
$parameterName =
...
assert(
$tagValue instanceof ExtendsTagValueNode
|| $tagValue instanceof ImplementsTagValueNode
|| $tagValue instanceof UsesTagValueNode,
);
$kind =
$tagValue
->type
...
->getAttribute('kind');
assert(
$kind instanceof IdentifierKind,
);
...
assert(
$refInfo
instanceof ClassLikeReferenceInfo,
);
$refInfo->genericArgs =
$tagValue
->type
...
*/
protected function processNameList(
array $names,
array $tagValues = [],
array $tagNames = [],
): array
...
* @return ClassLikeReferenceInfo[] indexed by [classLikeName]
*/
protected function processMixinTags(
array $values,
): array
{
$nameMap = [];
...
protected function processTypeOrNull(
...
$identifier->setAttribute(
'kind',
IdentifierKind::Keyword,
);
}
return $identifier;
}
protected function processExprOrNull(
?Node\Expr $expr,
): ?ExprInfo
{
return $expr
...
protected function processExpr(
Node\Expr $expr,
): ExprInfo
{
...
protected function extractPhpDoc(
Node $node,
): PhpDocNode
{
return $node->getAttribute('phpDoc')
?? new PhpDocNode([]);
}
/**
* @return PhpDocTextNode[] indexed by []
*/
protected function extractMultiLineDescription(
PhpDocNode $node,
): array
{
$textNodes = [];
...
* @return PhpDocTextNode[] indexed by []
*/
protected function extractSingleLineDescription(
...
*/
protected function extractGenericParameters(
PhpDocNode $node,
): array
{
$genericParameters = [];
...
&& $child->value
instanceof TemplateTagValueNode
) {
$lower =
strtolower(
$child->value->name,
...
GenericParameterVariance::Covariant,
str_ends_with(
$child->name,
'-contravariant',
) =>
GenericParameterVariance::Contravariant,
default =>
GenericParameterVariance::Invariant,
};
$genericParameters[$lower] =
...
$child instanceof PhpDocTagNode
&& $child->value
instanceof TypeAliasTagValueNode
) {
$lower =
strtolower(
$child->value->alias,
...
$aliases[$lower]->startLine =
$startLine;
$aliases[$lower]->endLine =
$endLine;
}
}
return $aliases;
...
if (
!$tag->value
instanceof InvalidTagValueNode
) {
$tags[
substr(
$tag->name,
...
*/
protected function extractParamTagValues(
PhpDocNode $node,
): array
{
$values = [];
...
$child instanceof PhpDocTagNode
&& $child->value
instanceof ParamTagValueNode
) {
$values[
$child->value->parameterName
] = $child->value;
}
}
return $values;
...
$value
instanceof ClassLikeReferenceInfo
&& $value->fullLower !== 'self'
&& $value->fullLower !== 'parent'
) {
$dependencies[
$value->fullLower
] ??= $value;
